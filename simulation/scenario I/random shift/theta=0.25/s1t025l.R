rm(list=ls())
setwd('/home/whiskeyindia7/theta/s1')
...
N.perm<-499;alpha<-0.05;radius<-0.50;theta<-0.25
...
for(a in 1:n.iter){
  ...
  if(a%%100==0){
    ...
    rej.test.lin.a<-colMeans(rej.test.lin[1:a,])
    rej.test.nw.a<-colMeans(rej.test.nw[1:a,])
    rej.test.nl.a<-colMeans(rej.test.nl[1:a,])
    rej.test.lin.tau.a<-colMeans(rej.test.lin.tau[1:a,])
    rej.test.nw.tau.a<-colMeans(rej.test.nw.tau[1:a,])
    rej.test.nl.tau.a<-colMeans(rej.test.nl.tau[1:a,])
    rej.test.lin.dcov.a<-colMeans(rej.test.lin.dcov[1:a,])
    rej.test.nw.dcov.a<-colMeans(rej.test.nw.dcov[1:a,])
    rej.test.nl.dcov.a<-colMeans(rej.test.nl.dcov[1:a,])

    names(rej.test.lin.a)<-names(rej.test.nw.a)<-names(rej.test.nl.a)<-colnames(X)
    names(rej.test.lin.tau.a)<-names(rej.test.nw.tau.a)<-names(rej.test.nl.tau.a)<-colnames(X)
    names(rej.test.lin.dcov.a)<-names(rej.test.nw.dcov.a)<-names(rej.test.nl.dcov.a)<-colnames(X)

    R<-list(rej.test.lin=rej.test.lin.a,
            rej.test.nw=rej.test.nw.a,
            rej.test.nl=rej.test.nl.a,
            rej.test.lin.tau=rej.test.lin.tau.a,
            rej.test.nw.tau=rej.test.nw.tau.a,
            rej.test.nl.tau=rej.test.nl.tau.a,
            rej.test.lin.dcov=rej.test.lin.dcov.a,
            rej.test.nw.dcov=rej.test.nw.dcov.a,
            rej.test.nl.dcov=rej.test.nl.dcov.a)

    filename<-paste0("s1_lin_theta025_iter", a, ".RData")
